...
pub type ScheduleSystem<I = (), O = ()> = SystemArc<dyn System<In = I, Out = O>>;

/// Type alias for a [`SystemArc`] that a [`Schedule`] can use as a condition.
///
/// [`Schedule`]: crate::schedule::Schedule
pub type ScheduleCondition<I = ()> = SystemArc<dyn ReadOnlySystem<In = I, Out = bool>>;
...
impl<S: System> From<S> for SystemArc<dyn System<In = S::In, Out = S::Out>> {
    /// Wraps the given system into an erased `SystemArc`.
...
impl<S: ReadOnlySystem> From<S> for SystemArc<dyn ReadOnlySystem<In = S::In, Out = S::Out>> {
    /// Wraps the given read-only system into an erased `SystemArc`.
    fn from(system: S) -> Self {
        SystemArc {
            system: Arc::new(Mutex::new(system)),
        }
