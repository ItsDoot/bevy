...
use crate::{
    ...
    schedule::{is_apply_deferred, BoxedCondition},
    schedule2::{DefaultGraph, DefaultGraphExecutable, ExecutorKind, ScheduleExecutor},
    world::World,
};
...
impl ScheduleExecutor<DefaultGraph> for SimpleExecutor {
    ...
    fn initialize(&mut self, executable: &DefaultGraphExecutable) {
    ...
    fn run(
        ...
        for system_index in 0..executable.systems.len() {
            ...
            if should_run {
                let valid_params = system.0.validate_param(world);
            ...
            let f = AssertUnwindSafe(|| {
                if let Err(err) = __rust_begin_short_backtrace::run(&mut system.0, world) {
                    error_handler(
                        err,
                        SystemErrorContext {
                            name: system.0.name(),
                            last_run: system.0.get_last_run(),
                        },
                    );
                }
            });
            ...
            #[cfg(feature = "std")]
            {
                if let Err(payload) = std::panic::catch_unwind(f) {
                    eprintln!("Encountered a panic in system `{}`!", &*system.0.name());
...
fn skip_automatic_sync_points() {
    ...
    use crate::{
        schedule2::{IntoChainableNodeConfigs, Schedule, ScheduleLabel},
        system::Commands,
    };
    ...
    let mut sched = Schedule::new(TestSchedule);
