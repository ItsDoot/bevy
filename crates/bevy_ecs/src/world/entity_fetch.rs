...
use crate::{
    ...
    world::{
        ...
        EntityWorldMut, Exclusive, Full, Shared,
    },
};
...
unsafe impl WorldEntityFetch for Entity {
    ...
    unsafe fn fetch_ref(self, cell: UnsafeWorldCell<'_>) -> Result<Self::Ref<'_>, Entity> {
        ...
        Ok(unsafe { EntityRef::new(ecell, Full, Shared) })
    }
    ...
    unsafe fn fetch_deferred_mut(
        ...
        let ecell = cell
            ...
            .ok_or(EntityFetchError::NoSuchEntity(self, cell))?;
        // SAFETY: caller ensures that the world cell has mutable access to the entity.
        Ok(unsafe { EntityMut::new(ecell, Full, Exclusive) })
    }
}
...
unsafe impl<const N: usize> WorldEntityFetch for &'_ [Entity; N] {
    ...
    unsafe fn fetch_ref(self, cell: UnsafeWorldCell<'_>) -> Result<Self::Ref<'_>, Entity> {
        ...
        for (r, &id) in core::iter::zip(&mut refs, self) {
            ...
            // SAFETY: caller ensures that the world cell has read-only access to the entity.
            *r = MaybeUninit::new(unsafe { EntityRef::new(ecell, Full, Shared) });
        }
    ...
    unsafe fn fetch_mut(
        ...
        for (r, &id) in core::iter::zip(&mut refs, self) {
            ...
            // SAFETY: caller ensures that the world cell has mutable access to the entity.
            *r = MaybeUninit::new(unsafe { EntityMut::new(ecell, Full, Exclusive) });
        }
...
unsafe impl WorldEntityFetch for &'_ [Entity] {
    ...
    unsafe fn fetch_ref(self, cell: UnsafeWorldCell<'_>) -> Result<Self::Ref<'_>, Entity> {
        ...
        for &id in self {
            let ecell = cell.get_entity(id).ok_or(id)?;
            // SAFETY: caller ensures that the world cell has read-only access to the entity.
            refs.push(unsafe { EntityRef::new(ecell, Full, Shared) });
        }
    ...
    unsafe fn fetch_mut(
        ...
        for &id in self {
            let ecell = cell
                ...
                .ok_or(EntityFetchError::NoSuchEntity(id, cell))?;
            // SAFETY: caller ensures that the world cell has mutable access to the entity.
            refs.push(unsafe { EntityMut::new(ecell, Full, Exclusive) });
        }
...
unsafe impl WorldEntityFetch for &'_ EntityHashSet {
    ...
    unsafe fn fetch_ref(self, cell: UnsafeWorldCell<'_>) -> Result<Self::Ref<'_>, Entity> {
        ...
        for &id in self {
            let ecell = cell.get_entity(id).ok_or(id)?;
            // SAFETY: caller ensures that the world cell has read-only access to the entity.
            refs.insert(id, unsafe { EntityRef::new(ecell, Full, Shared) });
        }
        Ok(refs)
    }
    ...
    unsafe fn fetch_mut(
        ...
        for &id in self {
            let ecell = cell
                ...
                .ok_or(EntityFetchError::NoSuchEntity(id, cell))?;
            // SAFETY: caller ensures that the world cell has mutable access to the entity.
            refs.insert(id, unsafe { EntityMut::new(ecell, Full, Exclusive) });
        }
        Ok(refs)
    }
