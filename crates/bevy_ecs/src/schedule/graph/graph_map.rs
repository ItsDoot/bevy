...
impl<const DIRECTED: bool, N: GraphNodeId, S: BuildHasher> Graph<DIRECTED, N, S> {
    ...
    /// Remove a node `n` from the graph. Returns `true` if the node was present
    /// and removed.
    ///
...
impl<N: GraphNodeId, S: BuildHasher> DiGraph<N, S> {
    /// Tries to topologically sort this graph.
    ///
    ...
    /// # Errors
    ///
    /// If the graph contains cycles, returns [`GraphToposortError`].
    pub fn toposort(&self) -> Result<Vec<N>, GraphToposortError<N>> {
        // Check explicitly for self-edges.
        // `iter_sccs` won't report them as cycles because they still form components of one node.
        if let Some((node, _)) = self.all_edges().find(|(a, b)| a == b) {
            return Err(GraphToposortError::Loop(node));
        }
        ...
        if sccs_with_cycles.is_empty() {
            ...
            Err(GraphToposortError::Cycles(cycles))
        }
    }
...
#[derive(Error, Debug)]
pub enum GraphToposortError<N: GraphNodeId> {
    #[error("graph contains a node that loops on itself: {0}")]
    Loop(N),
    #[error("graph contains cycles in the following strongly-connected components: {0:?}")]
    Cycles(Vec<Vec<N>>),
}
