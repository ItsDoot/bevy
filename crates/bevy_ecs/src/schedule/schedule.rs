...
pub struct Schedule {
    ...
    warnings: Vec<ScheduleBuildError>,
}
...
impl Schedule {
    ...
    /// [`Schedule::initialize`].
    pub fn warnings(&self) -> &[ScheduleBuildError] {
        &self.warnings
    }
}
...
impl ScheduleGraph {
    ...
    pub fn build_schedule(
    ...
    ) -> Result<(SystemSchedule, Vec<ScheduleBuildError>), ScheduleBuildError> {
        let mut warnings = Vec::new();

        // check hierarchy for cycles
        let hierarchy = self.hierarchy.toposort()?;
        let hierarchy_analysis = hierarchy.analyze();
        if let Some(warning) =
            self.optionally_check_hierarchy_conflicts(&hierarchy_analysis.transitive_edges)?
        {
        ...
        let dependency = self.dependency.toposort()?;
        ...
        self.check_for_cross_dependencies(&dep_results, &hierarchy_analysis.connected)?;
    ...
    fn get_dependency_flattened(
        ...
        let mut dependency_flattening = self.dependency.clone();
    ...
    fn update_schedule(
        ...
        ignored_ambiguities: &BTreeSet<ComponentId>,
    ) -> Result<Vec<ScheduleBuildError>, ScheduleBuildError> {
...
impl ScheduleGraph {
    ...
    ) -> Result<Option<ScheduleBuildError>, ScheduleBuildError> {
        match (
            self.settings.hierarchy_detection,
            transitive_edges.is_empty(),
        ) {
            (LogLevel::Warn, false) => Ok(Some(ScheduleBuildError::HierarchyRedundancy(
                transitive_edges.to_vec(),
            ))),
            (LogLevel::Error, false) => Err(ScheduleBuildError::HierarchyRedundancy(
                transitive_edges.to_vec(),
            )),
            _ => Ok(None),
        }
    ...
    fn optionally_check_conflicts(
        ...
        match (self.settings.ambiguity_detection, conflicts.is_empty()) {
            (LogLevel::Warn, false) => Ok(Some(ScheduleBuildError::Ambiguity(conflicts.to_vec()))),
            (LogLevel::Error, false) => Err(ScheduleBuildError::Ambiguity(conflicts.to_vec())),
            _ => Ok(None),
        }
    }
...
pub enum ScheduleBuildError {
    ...
    #[error("system (set) `{0:?}` has been told to run before itself")]
    DependencyLoop(NodeId),
    /// The dependency graph contains a cycle.
    #[error("the dependency graph contains a cycle: {0:?}")]
    DependencyCycle(Vec<Vec<NodeId>>),
    /// Tried to order a system (set) relative to a system set it belongs to.
    #[error("tried to order a system (set) `{0:?}` relative to a system set it belongs to")]
    CrossDependency(NodeId, NodeId),
    /// Tried to order system sets that share systems.
    #[error("tried to order system sets `{0:?}` and `{1:?}` that share systems")]
    SetsHaveOrderButIntersect(SystemSetKey, SystemSetKey),
    /// Tried to order a system (set) relative to all instances of some system function.
    #[error(
        "tried to order a system (set) `{0:?}` relative to all instances of some system function"
    )]
...
impl ScheduleBuildError {
    ...
    fn hierarchy_cycle_to_string(cycles: &[Vec<NodeId>], graph: &ScheduleGraph) -> String {
        ...
        for (i, cycle) in cycles.iter().enumerate() {
            let mut names = cycle.iter().map(|id| graph.get_node_name(id));
    ...
    fn sets_have_order_but_intersect_to_string(
        ...
        format!(
            "tried to order system sets `{}` and `{}` that share systems",
    ...
    fn system_type_set_ambiguity_to_string(set: &SystemSetKey, graph: &ScheduleGraph) -> String {
        format!(
            "tried to order system set `{}` relative to all instances of some system function",
            graph.get_node_name(&NodeId::Set(*set))
        )
    }
